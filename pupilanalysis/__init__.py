...
Python pacakge `pupilanalysis` provides an intergrated set of graphical
...
More precesily, `pupilanalysis` takes advantage of the following data structuring
...
Please mind that `pupilanalysis` was created to analyse <em>Drosophila</em>'s
deep pseudopupil movement and orientation data across its left and right eyes
...
Currently `pupilanalysis` comes with two user interfaces, one being command line based and
the other graphical.

The documentation on this page gives out only general instrctions and ideas.
To have exact usage instructions, please refer to submodule documentation of `pupilanalysis.drosom.terminal` and
`pupilanalysis.tkgui`.
...
In `pupilanalysis`, rectangular ROIs are manually drawn by the user
...
Using 2D cross-correlation based methods (template matching), `pupilanalysis`
...
At the moment, `pupilanalysis` supports only one rotation stage configuration that is
...
you can override the `pupilanalysis.drosom.analysing.MAnalyser` with `pupilanalysis.drosom.orientation_analysis.OAnalyser`,
making the movement measurement part to be a manual user drawing arrows according to the feature direction analysis.
Then later, you can use any `pupilanalysis` code paths just remembering that for example in the vectormap,
...
All quitely saved files are stored in a location set in `pupilanalysis.directories`.
By default, on Windows this is <em>C:/Users/USER/PupilAnalysis</em>
and on other platforms <em>/home/USER/.pupilanalysis</em>


Command line interface
----------------------

`pupilanalysis` also includes a command interface, that can be invoked by
